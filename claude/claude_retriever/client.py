...
class ClientWithRetrieval(Anthropic):
    ...
    def answer_with_results(self,
                            raw_search_results: List[str] | List[SearchResult],
                            query: str,
                            model: str,
                            temperature: float,
        ...
        if isinstance(raw_search_results[0], str):
            search_results = [SearchResult(content=s) for s in
                              raw_search_results]  # type: ignore

        if format_results:
            processed_search_results = [search_result.content.strip() for
                                        search_result in
                                        search_results]  # type: ignore
            formatted_search_results = format_results_full(
    ...
    def completion_with_retrieval(self,
        ...
        search_results = self.retrieve(query, model=model,
                                       ...
                                       temperature=temperature)
        answer = self.answer_with_results(search_results, query, model,
                                          temperature)
        return answer

    # Helper methods
    def _search_query_stop(self, partial_completion: str,
                           n_search_results_to_use: int) -> Tuple[
        ...
        search_query = self.extract_between_tags('search_query',
                                                 partial_completion + '</search_query>')
        ...
        search_results = self.search_tool.raw_search(search_query,
    ...
    def extract_between_tags(self, tag, string, strip=True):
        ...
        """
        ext_list = re.findall(f"<{tag}\\s?>(.+?)</{tag}\\s?>", string,
                              re.DOTALL)
        if strip:
